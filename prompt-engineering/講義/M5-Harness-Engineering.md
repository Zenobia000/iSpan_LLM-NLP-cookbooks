...
別急著理解「harness」這個詞。先看一個 agent 跑起來長什麼樣，它和你前四章寫的「一次請求、一次回答」差在哪：

```python
# 概念骨架（真正可跑版見 01-function-calling-agents）
# 一個最小 agent：模型自己決定要不要呼叫工具，迴圈到它說「做完了」
tools = [get_weather]                       # 只給它一個查天氣的工具
messages = [{"role": "user", "content": "台北現在適合穿什麼？"}]

while True:
    resp = 問模型(messages, tools)           # 模型看脈絡，決定下一步
    if resp 沒有要求呼叫工具:                 # 模型改吐純文字 = 任務完成
        print(resp 的文字答案); break
    for call in resp 要呼叫的工具:            # 模型自己決定呼叫誰
        result = 執行(call)                  # 你負責執行
        messages.append(把 result 寫回脈絡)   # 結果寫回，再問一次
```

跑下去你會看到：模型先要求呼叫 `get_weather`，拿到「18°C」後，自己決定不用再查了，直接回答「帶件外套」。**整段過程沒有任何一句 prompt 在指揮它「第一步查天氣、第二步…」，是模型自己在這個 while 迴圈裡決定下一步。**

這就是 agent 的全部本質：**一個 while 迴圈**。而這一章只回答一個問題：這個迴圈搬到生產環境時，憑什麼不崩？工具會 timeout、脈絡會爆掉、會有人惡意注入指令。把這幾個會崩的點各補上工程保證，就是 **harness**（白話：一副給 AI 用的「馬具」，讓它穩定發揮的受控環境）。那幾個點，就是 §3 的五大架構。
...
回到 §0 那個 while 迴圈：模型看脈絡 → 決定呼叫哪個工具 → 工具回傳結果寫回脈絡 → 模型再看一次 → 直到它判斷任務完成。Harness Engineering 就是把這個迴圈的每一個環節，從「靠 prompt 祈禱」升級成「靠工程保證」。下面五大架構，正是這個迴圈的五個受控點。
