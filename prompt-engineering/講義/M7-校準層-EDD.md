...
## 0. 先用一個數字取代「感覺有變好」，再談成熟度

改了一行 system prompt，下次回答看起來順了，你就以為修好了；其實你只是換了一組沒被測到的失敗。這一章要終結這種自我欺騙，但別急著理解後面那張成熟度地圖。先做一個最簡單、現在就能動手的動作：拿一個數字，把「感覺有變好」換掉。

做法只有三步：拿這個 prompt 的 10 筆輸出，每一筆人工判一個「對 / 錯」，然後把對的筆數除以總筆數。

```python
# 概念骨架（真正可跑版見 01-rag-evaluation、02-feedback-loop）
# 拿 10 筆輸出，每筆人工判「對 / 錯」，算出一個通過率
outputs = 跑 10 次拿到的 10 筆輸出
verdicts = [人工判對嗎(o) for o in outputs]   # 每筆只回 True / False，不打分數
pass_rate = sum(verdicts) / len(verdicts)      # 例如 6 對 4 錯 → 0.6
print(pass_rate)                               # 一個可重複、能比較的數字
```

跑下去你會拿到一個數字，例如 `0.6`。下次你改了 prompt，再跑同一份 10 筆、再算一次，得到 `0.85`。**這兩個數字之間的差，就是你第一次用「量測」證明改善，而不是用「感覺」宣稱改善。** 這個動作有個名字叫 **EDD**（白話：評估驅動開發 = 每次改動都用一個可重複的分數證明它真的變好，而不是憑感覺）；裡頭那個「只判對 / 錯、不打 1–10 分」的關鍵，叫**二元判斷**（白話：別給 1–10 分，只問對或錯，因為對錯才可重複，1–10 分連你自己隔天都打不一樣）。

你剛做的這件事，在評估成熟度的階梯上正好是 **L2**：固定一份 dataset、明確判準、逐筆二元評分。往下還有「自己看一眼覺得順」的 L0、往上還有「CI 自動跑」「機器自動優化」的 L3、L4。下一節這張 5-Level 地圖，就是把你剛動手做過的這格放回整條路上，讓你看清自己現在在哪、下一步往哪走。
...
§0 你動手算出的那個通過率，就是這張地圖上的 **L2**。評估不是「有沒有做」的開關，而是一條成熟度階梯。把剛做過的那格放回整條路上，先定位自己在哪一格，才知道下一步往哪推。
...
**合成資料轉 chat JSONL。** 訓練資料的格式是每行一個 JSON 物件，內含一段 `messages`（system / user / assistant 三角色）。資料哪來？同樣用 3.4 的合成法：對文件 chunk 產生 (question, answer)，再包成 chat 格式：
